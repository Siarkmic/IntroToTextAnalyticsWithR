setwd("C:/Users/siarkmi2/OneDrive dla firm 1/NEW_WORLD_ORDER/TEXT_MINE")
...
optOut.raw <- read.csv("C:/Users/siarkmi2/Documents/R/opt-out.csv", 
                       stringsAsFactors = FALSE,
                     sep = ';', quote ="\"")
#View(optOut.raw)

# Clean up the data frame and view our handiwork.
optOut.raw$Label <- optOut.raw$czy.opt.out 
optOut.raw$Text <- optOut.raw$RC_DESCRLONG 

myvars <- c("Label", "Text")
optOut <- optOut.raw[myvars]

#View(optOut)

# Remove character(0) from data frame
optOut <- subset(optOut, Text!="")

#View(optOut)


# Check data to see if there are missing values.
length(which(!complete.cases(optOut)))

# Convert our class label into a factor.
optOut$Label <- as.factor(optOut$Label)
...
optOut$TextLength <- nchar(optOut$Text)
summary(optOut$TextLength)
...
ggplot(optOut, aes(x = TextLength, fill = Label)) +
        theme_bw() +
...
set.seed(32984)
indexes <- createDataPartition(optOut$Label, times = 1,
                               p = 0.99, list = FALSE)

test <- optOut[indexes,]
train <- optOut[-indexes,]
...
#        your problem/domain.
train.tokens <- tokens_select(train.tokens, stopwords(SMART), 
                              selection = "remove")
train.tokens[[357]]
...
dfm(train.tokens, remove = stopwordsPL, stem = FALSE, remove_punct = TRUE,
    ngrams=c(1,2))

train.tokens[[357]]

# form the tokens, removing punctuation
mytoks <- tokens(train.tokens, remove_punct = TRUE)
# remove the Polish stopwords, leave pads
mytoks <- tokens_remove(train.tokens, stopwordsPL, padding = TRUE)
...
mytoks <- tokens_remove(train.tokens, stopwordsPL, padding = FALSE)

train.tokens <- mytoks

# Perform stemming on the tokens.
train.tokens <- tokens_wordstem(train.tokens, language = "polish")
tokens_wordstem()
